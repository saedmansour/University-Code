...
public:
	// TODO: check?
	TreeNode(): RightSon(NULL), LeftSon(NULL), Father(NULL)/*, Data(NULL)*/, Height(0),Updated(1),RightSons(0),LeftSons(0){}
	// TODO:
...
public:
	...
	void getAllHelper(R** KeyArray, S** DataArray, int* index, TreeNode<S, R>* node)
	{
		if(node == NULL){
			(*index)--;
			return;
	...
	int getElementsNum(TreeNode<S, R>* node)
	{
		if(node == NULL)
		{
			return 0;
